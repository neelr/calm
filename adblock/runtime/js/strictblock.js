...
(async ( ) => {
    ...
    for ( const details of rulesetDetails ) {
        ...
        toImport.push(`/adblock/runtime/rulesets/scripting/popup/${details.id}.js`);
    }
    if ( toImport.length === 0 ) { return; }
    await Promise.all(toImport.map(a => import(a)));
    self.preventPopupTarget = toURL;
    await import('/adblock/runtime/js/scripting/prevent-popup.js');
})();
...
(async ( ) => {
    ...
    const searchInList = async i => {
        if ( iList !== -1 ) { return; }
        const rules = await fetchJSON(`/adblock/runtime/rulesets/strictblock/${rulesetDetails[i].id}`);
        if ( iList !== -1 ) { return; }
...
(async ( ) => {
    ...
    for ( const details of rulesetDetails ) {
        ...
        toFetch.push(fetchJSON(`/adblock/runtime/rulesets/urlskip/${details.id}`));
    }
    if ( toFetch.length === 0 ) { return; }
