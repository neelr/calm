...
function getRulesetDetails() {
    ...
    getRulesetDetails.rulesetDetailsPromise =
        fetchJSON('/adblock/runtime/rulesets/ruleset-details').then(entries => {
...
async function updateRegexRules(currentRules, addRules, removeRuleIds) {
    ...
    for ( const details of rulesetDetails ) {
        ...
        toFetch.push(fetchJSON(`/adblock/runtime/rulesets/regex/${details.id}`));
    }
...
async function updateStrictBlockRules(currentRules, addRules, removeRuleIds) {
    ...
    for ( const details of rulesetDetails ) {
        ...
        toFetch.push(fetchJSON(`/adblock/runtime/rulesets/strictblock/${details.id}`));
    }
    const rulesets = await Promise.all(toFetch);

    const substitution = `${runtime.getURL('/adblock/runtime/strictblock.html')}#\\0`;
    const allRules = [];
