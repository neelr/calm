...
function getScriptletDetails() {
    ...
    promise = fetchJSON('/adblock/runtime/rulesets/scriptlet-details').then(
...
function getGenericDetails() {
    ...
    if ( promise !== undefined ) { return promise; }
    promise = fetchJSON('/adblock/runtime/rulesets/generic-details').then(
        entries => new Map(entries)
    );
...
function registerHighGeneric(context, genericDetails) {
    ...
    for ( const details of rulesetsDetails ) {
        ...
        if ( count === 0 ) { continue; }
        css.push(`/adblock/runtime/rulesets/scripting/generichigh/${details.id}.css`);
    }
...
function registerGeneric(context, genericDetails) {
    ...
    for ( const details of rulesetsDetails ) {
        ...
        if ( count === 0 ) { continue; }
        js.push(`/adblock/runtime/rulesets/scripting/generic/${details.id}.js`);
    }

    if ( js.length === 0 ) { return; }

    js.unshift('/adblock/runtime/js/scripting/css-api.js', '/adblock/runtime/js/scripting/isolated-api.js');
    js.push('/adblock/runtime/js/scripting/css-generic.js');
...
async function registerCosmetic(realm, context) {
...

    {
        ...
        for ( const id of rulesetIds ) {
            promises.push(
                fetchJSON(`/adblock/runtime/rulesets/scripting/${realm}/${id}`).then(data => {
    ...
    const js = rulesetIds.map(id => `/adblock/runtime/rulesets/scripting/${realm}/${id}.js`);
    js.unshift('/adblock/runtime/js/scripting/css-api.js', '/adblock/runtime/js/scripting/isolated-api.js');
    js.push(`/adblock/runtime/js/scripting/${realmid}.js`);
...
function registerScriptlet(context, scriptletDetails) {
    ...
    for ( const rulesetId of rulesetsDetails.map(v => v.id) ) {
        ...
        for ( const world of Object.keys(worlds) ) {
            ...
            const directive = {
                id,
                js: [ `/adblock/runtime/rulesets/scripting/scriptlet/${world.toLowerCase()}/${rulesetId}.js` ],
                matches,
                allFrames: true,
